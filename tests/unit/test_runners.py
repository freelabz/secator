...
class TestIsOwnSource(unittest.TestCase):
	"""Tests for Runner._is_own_source and the prior-error isolation fix (issue #987)."""

	def _make_cmd(self, name='nmap', chunk=None):
		"""Create a minimal MyCommand instance with the given name / chunk."""
		opts = {}
		if chunk is not None:
			opts['chunk'] = chunk
		with mock_command(MyCommand, TARGETS, opts, []) as cmd:
			# Override name / unique_name to simulate arbitrary task names.
			cmd.name = name
			cmd.unique_name = name.replace('/', '_')
			if chunk is not None:
				cmd.unique_name = f'{cmd.unique_name}_{chunk}'
			return cmd

	# ------------------------------------------------------------------
	# _is_own_source unit tests
	# ------------------------------------------------------------------

	def test_own_source_exact_match(self):
		cmd = self._make_cmd('nmap')
		self.assertTrue(cmd._is_own_source('nmap'))

	def test_own_source_numeric_chunk(self):
		cmd = self._make_cmd('nmap')
		self.assertTrue(cmd._is_own_source('nmap_1'))
		self.assertTrue(cmd._is_own_source('nmap_42'))

	def test_own_source_prefix_only_is_excluded(self):
		"""'nmap_light' must NOT be considered an own source of 'nmap'."""
		cmd = self._make_cmd('nmap')
		self.assertFalse(cmd._is_own_source('nmap_light'))

	def test_own_source_different_name_excluded(self):
		cmd = self._make_cmd('nuclei')
		self.assertFalse(cmd._is_own_source('nmap'))
		self.assertFalse(cmd._is_own_source('nmap_light'))
		self.assertFalse(cmd._is_own_source('nmap_1'))

	def test_own_source_empty_string(self):
		cmd = self._make_cmd('nmap')
		self.assertFalse(cmd._is_own_source(''))

	# ------------------------------------------------------------------
	# self_errors isolation: prior errors must not block a task
	# ------------------------------------------------------------------

	def test_self_errors_excludes_different_task_errors(self):
		"""Errors from a previous task with a different name must not appear in self_errors."""
		with mock_command(MyCommand, TARGETS, {}, []) as cmd:
			prior_error = Error(message='nmap failed')
			prior_error._source = 'nmap'
			cmd.results.append(prior_error)
			self.assertEqual(cmd.self_errors, [])

	def test_self_errors_excludes_prefix_matched_task_errors(self):
		"""Errors from 'nmap_light' must not appear in a task named 'nmap' (issue #987 prefix bug)."""
		with mock_command(MyCommand, TARGETS, {}, []) as cmd:
			cmd.name = 'nmap'
			cmd.unique_name = 'nmap'
			prior_error = Error(message='nmap_light failed')
			prior_error._source = 'nmap_light'
			cmd.results.append(prior_error)
			self.assertEqual(cmd.self_errors, [])

	def test_self_errors_includes_own_errors(self):
		"""Errors generated by this task's own initialisation must appear in self_errors."""
		with mock_command(MyCommand, TARGETS, {}, []) as cmd:
			own_error = Error(message='validation failed')
			own_error._source = cmd.unique_name
			cmd.results.append(own_error)
			self.assertEqual(len(cmd.self_errors), 1)

	def test_self_errors_includes_chunk_errors(self):
		"""Errors from this task's numeric chunks must appear in self_errors."""
		with mock_command(MyCommand, TARGETS, {}, []) as cmd:
			chunk_error = Error(message='chunk failed')
			chunk_error._source = f'{cmd.unique_name}_1'
			cmd.results.append(chunk_error)
			self.assertEqual(len(cmd.self_errors), 1)

	# ------------------------------------------------------------------
	# __iter__ guard: prior errors must not prevent a task from running
	# ------------------------------------------------------------------

	def test_task_runs_despite_prior_different_task_error(self):
		"""A task must run even when prior results contain errors from a different task."""
		fixture = ['{"url": "http://example.com"}']
		MyCommand.output_types = [Url]
		MyCommand.item_loaders = [JSONSerializer()]

		prior_error = Error(message='previous task failed')
		prior_error._source = 'some_other_task'

		with mock_command(MyCommand, TARGETS, {'results': [prior_error]}, fixture) as cmd:
			results = cmd.run()
			urls = [r for r in results if isinstance(r, Url)]
			# The prior error is passed through but must not block the task or count as own error.
			own_errors = [r for r in results if isinstance(r, Error) and r._source == cmd.unique_name]
			self.assertEqual(len(urls), 1, 'task must still produce its own results')
			self.assertEqual(len(own_errors), 0, 'no errors from this task expected')
			self.assertEqual(cmd.status, 'SUCCESS')

		MyCommand.output_types = []
		MyCommand.item_loaders = []

	def test_task_runs_despite_prior_prefix_matched_error(self):
		"""A task named 'nmap' must run even when prior results have 'nmap_light' errors."""
		fixture = ['{"url": "http://example.com"}']
		MyCommand.output_types = [Url]
		MyCommand.item_loaders = [JSONSerializer()]
		...
		# Pass the prior error as results= so the command is initialized with it.
		with mock_command(MyCommand, TARGETS, {'results': [prior_error]}, fixture) as cmd:
			# Simulate running as 'nmap' (which shares prefix with 'nmap_light').
			cmd.unique_name = 'nmap'
			results = cmd.run()
			urls = [r for r in results if isinstance(r, Url)]
			self.assertEqual(len(urls), 1, 'nmap must still run and produce results')
			self.assertEqual(cmd.status, 'SUCCESS')

		MyCommand.output_types = []
		MyCommand.item_loaders = []

	def test_workflow_runner_self_errors_includes_all_errors(self):
		"""For non-task runners, self_errors must return all errors (workflow status semantics)."""
		from secator.runners._base import Runner
		from dotmap import DotMap

		# Build a bare runner object with workflow config, bypassing full __init__.
		runner = Runner.__new__(Runner)
		runner.config = DotMap({'type': 'workflow'})
		runner.unique_name = 'test_workflow'
		runner.results = []

		task_error = Error(message='task failed')
		task_error._source = 'nmap'
		runner.results.append(task_error)

		self.assertEqual(len(runner.self_errors), 1, 'workflow self_errors must include task errors')
