...
impl LoadedPlugins {
    /// Merge another set of loaded plugins into this one. Used by the CLI to
    /// combine the scaffold-templates result with the custom_templates repos.
...
/// Walk `<dir>/target/release/` for cdylibs and dlopen each one. Any failure
/// (missing symbols, ABI mismatch, IO error) is logged + skipped — one bad
/// plugin never fails the whole load. Returns the aggregate of every
/// contribution every loaded plugin registered.
///
/// The `dir` argument names the crate root — either `<dirs.templates>` (for
/// scaffolded plugins) or a single `<custom>/<repo>` (for git-cloned template
/// packs). Both layouts share the same `target/release/` convention.
pub fn load_user_plugins(dir: &Path) -> LoadedPlugins {
