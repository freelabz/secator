...
def get_system_prompt(mode: str, workspace_path: str = "", backend=None) -> str:
	...
	if backend is not None:
		...
		if "follow_up" in excluded:
			interaction_rules = STOP_RULES
		elif "stop" in excluded:
			interaction_rules = FOLLOW_UP_RULES
	else:
		interaction_rules = FOLLOW_UP_RULES
	...
	# Append interaction rules after common constraints
	if interaction_rules:
		result += interaction_rules
