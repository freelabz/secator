...
class CeleryPoller(object):
	...
	def iter_results(
		...
		with progress:
			...
			if print_remote_info:
				progress_cache = CeleryPoller.init_progress(progress, ids_map)

			# Get live results and print progress
			for data in CeleryPoller.poll(result, ids_map, refresh_interval, revoked):
				...
				if print_remote_info:
					...
					CeleryPoller.update_progress(progress, progress_id, data)
					progress.refresh()
	...
	def poll(result, ids_map, refresh_interval, revoked=False):
		...
		while not exit_loop:
			try:
				yield from CeleryPoller.get_all_data(result, ids_map, revoked=revoked)
	...
	def get_all_data(result, ids_map, revoked=False):
		...
		yield from CeleryPoller.get_tasks_data(ids_map, revoked=revoked)
	...
	def get_tasks_data(ids_map, revoked=False):
		...
		for task_id in task_ids:
			data = CeleryPoller.get_task_data(task_id, ids_map)
			if not data:
				continue
	...
	def get_task_ids(result, ids=[]):
		...
		try:
			if isinstance(result, GroupResult):
				CeleryPoller.get_task_ids(result.parent, ids=ids)
			...
			if hasattr(result, 'children'):
				...
				if isinstance(children, list):
					for child in children:
						CeleryPoller.get_task_ids(child, ids=ids)
