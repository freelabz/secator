...
class Runner:
	...
	def __init__(self, config, inputs=[], results=[], run_opts={}, hooks={}, validators={}, context={}):
		...
		# Runner state
		self.uuids = set()
		# ponytail: bounded own-emissions buffer, the no-store FALLBACK for `results`. When a store
		# driver is active (the normal case — json default), `results` is the instance-less
		# StreamView and this stays the runner's own topology (never the fan-in). Delete it once
	...
	def _view(self, extra=None):
		"""StreamView of this runner's OWN subtree — findings/execution scoped by the runner's own
		{type}_id (a Task sees task_id, a Workflow workflow_id, a Scan the whole scan_id). Instance-
		less: rebuilt from context, so pickle-transparent. Returns None when no store/id is active
		(caller falls back to the in-memory own-emissions buffer)."""
		own_id = self.context.get(f'{self.config.type}_id')
		if not (self.context.get('drivers') and own_id):
			return None
	...
	@property
	def results(self):
		"""Instance-less streaming view of this runner's own subtree (scoped by its {type}_id),
		or the in-memory own-emissions buffer when no store is active."""
		v = self._view()
		return v if v is not None else self._results

	@property
	def targets(self):
		v = self._view({'_type': 'target'})
		return list(v) if v is not None else [r for r in self._results if isinstance(r, Target)]

	def _is_own_source(self, source):
		"""Return True when *source* was generated by this runner or one of its chunks.

		Accepts exact match (e.g. "nmap") and numeric chunk suffixes (e.g. "nmap_1",
		"nmap_2") while correctly rejecting same-prefix siblings like "nmap_light".
		"""
	...
	@property
	def infos(self):
		# The view is already scoped to this runner's subtree; a task's subtree == its own source.
		v = self._view({'_type': 'info'})
		if v is None:
			src = self.config.type == 'task'
			return [r for r in self._results if isinstance(r, Info) and (not src or self._is_own_source(r._source))]
		return list(v)

	@property
	def warnings(self):
		v = self._view({'_type': 'warning'})
		if v is None:
			src = self.config.type == 'task'
			return [r for r in self._results if isinstance(r, Warning) and (not src or self._is_own_source(r._source))]
		return list(v)
	...
	@property
	def errors(self):
		# Errors are few — materialize the type-scoped view and apply the ownership filter.
		v = self._view({'_type': 'error'})
		src = v if v is not None else [r for r in self._results if isinstance(r, Error)]
		return [r for r in src if self._owns_error(r)]
	...
	@property
	def findings(self):
		# Instance-less streaming view of this runner's own subtree's findings, scoped by its
		# {type}_id. Falls back to the in-memory own-findings only when no store is active.
		v = self._view({'_type': {'$in': [t.get_name() for t in FINDING_TYPES]}})
		return v if v is not None else [r for r in self._results if isinstance(r, tuple(FINDING_TYPES))]
	...
	def self_findings_count(self):
		# Live per-task count (celery update_state): a task's own {type}_id-scoped finding count.
		v = self._view({'_type': {'$in': [t.get_name() for t in FINDING_TYPES]}})
		if v is not None and self.config.type == 'task':
			return len(v)
		return len([r for r in self.self_findings])
	...
	def _run_extractors(self):
		...
		inputs, run_opts, errors = run_extractors(self._results, self.run_opts, self.inputs, ctx=ctx, dry_run=self.dry_run)
		for error in errors:
			self.add_result(error)
	...
	def add_result(self, item, print=True, output=True, hooks=True, queue=True):
		...
		# Fan out to the store via on_item (run above); keep the item in the bounded own-emissions
		# buffer too — the no-store fallback for `results` (and it captures hooks=False transients).
	...
	def _get_findings_count(self):
		...
		for output_type in FINDING_TYPES:
			...
			v = self._view({'_type': name})
			count = len(v) if v is not None else len([r for r in self._results if isinstance(r, output_type)])
			if count > 0:
