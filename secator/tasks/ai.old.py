...
class ai(PythonRunner):
	...
	def _run_loop(self, mode: str, prompt: str, targets: List[str], model: str,
		...
		# Resolve interactive option (string or legacy bool)
		interactive = self.run_opts.get("interactive")
		if interactive is True:
			interactive = "local"
		elif interactive is False:
			interactive = "auto"
		elif interactive is None:
			interactive = "local" if self.sync else "auto"
		...
		# Create interactivity backend (before history init for prompt generation)
		session_id = self.run_opts.get("name") or str(self.id)
		backend = create_backend(interactive, timeout=CONFIG.addons.ai.user_response_timeout)
		...
		if resumed_history:
			...
			history = ChatHistory()
			history.model = model
			system_prompt = get_system_prompt(mode, backend=backend)
		...
		if isinstance(backend, RemoteBackend):
			backend.query_engine = ctx.get_query_engine()

		# Build tool schemas for native tool calling
		tool_schemas = build_tool_schemas(mode, backend=backend)

		# When resuming, prompt user for new instructions before entering the loop
		if resumed_history and interactive != "auto":
		...
		while iteration < max_iter:
			...
			try:
				...
				if interactive == "local" and context_warnings and ctx_window > 0:
				...
				# Handle follow_up, no actions, or max_iter reached
				if follow_up_choices is not None or not actions or iteration == max_iter:
					if interactive == "auto":
						save_history(history, self.reports_folder, debug_fn=self.debug)
						return
					elif isinstance(backend, RemoteBackend) and follow_up_ai:
						# Enrich finding for remote polling
						follow_up_ai.status = "pending"
						follow_up_ai.session_id = session_id
						yield follow_up_ai
						# Poll DB for answer
						answer = backend.ask_user(
						...
						if answer is None:
							save_history(history, self.reports_folder, debug_fn=self.debug)
							return
						# Add answer to history and continue
						history.add_user(maybe_encrypt(answer, encryptor))
						continue
					else:
						# CLI backend: use existing prompt mechanism
						result = self._prompt_and_redetect(
