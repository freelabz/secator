...
class ffuf(HttpFuzzer):
	...
	def on_cmd_opts(self, opts):
		# Fuzz host header
		if self.get_opt_value('fuzz_host_header') and len(self.inputs) > 1:
		...
		if len(self.inputs) > 1 and 'FUZZ' not in self.inputs[0] and not fuzz_in_headers and 'FUZZ' not in data:
