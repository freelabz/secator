...
def tag_duplicates(ws_id: str = None, full_scan: bool = False):
	...
	for item in untagged_findings:
		...
		if duplicate_ws:
			...
			for item in duplicate_ws:
				related_ids.extend(item._related)
	...
	import json
	print('db updates:')
	print(json.dumps(db_updates, indent=2))
