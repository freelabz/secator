...
Configuration:
	- addons.discord.enabled: Enable/disable the Discord driver
	...
	for attempt in range(MAX_RETRIES):
		try:
			...
			if response.status_code == 429:
				...
				debug(f'rate limited, retrying in {retry_after}s (attempt {attempt + 1}/{MAX_RETRIES})', sub='drivers.discord')  # noqa: E501
			...
			if attempt < MAX_RETRIES - 1:
				debug(f'request failed (attempt {attempt + 1}/{MAX_RETRIES}): {e}', sub='drivers.discord')
				time.sleep(1)
			else:
				debug(f'request failed after {MAX_RETRIES} attempts: {e}', sub='drivers.discord')
	return None


class DiscordDriver(Driver):
	"""Discord driver. Sends runner and finding updates to a Discord webhook."""

	def __init__(
		self,
		webhook_url=None,
		bot_token=None,
		send_runner_updates=None,
		send_findings=None,
		finding_types=None,
		min_severity=None,
	):
		self.webhook_url = webhook_url or CONFIG.addons.discord.webhook_url
		self.bot_token = bot_token or CONFIG.addons.discord.bot_token
		self.send_runner_updates = send_runner_updates if send_runner_updates is not None else CONFIG.addons.discord.send_runner_updates  # noqa: E501
		self.send_findings = send_findings if send_findings is not None else CONFIG.addons.discord.send_findings
		self.finding_types = finding_types if finding_types is not None else CONFIG.addons.discord.finding_types
		self.min_severity = min_severity or CONFIG.addons.discord.min_severity
	...
	def _create_message(self, embeds):
		"""Create a new Discord webhook message. Returns (message_id, channel_id)."""
		if not self.webhook_url:
			debug('skipped: no webhook_url configured', sub='drivers.discord')
			return None, None
		response = _request_with_retry('POST', f'{self.webhook_url}?wait=true', json={'embeds': embeds})
		if response:
			data = response.json()
			msg_id = data.get('id')
			channel_id = data.get('channel_id')
			debug(f'message created: {msg_id}', sub='drivers.discord')
			return msg_id, channel_id
		return None, None

	def _edit_message(self, message_id, embeds, thread_id=None):
		...
		if response:
			debug(f'message updated: {message_id}', sub='drivers.discord')

	def _create_thread(self, channel_id, message_id, thread_name):
		...
		if response:
			...
			debug(f'thread created: {thread_id}', sub='drivers.discord')
			return thread_id
		return None

	def _post_to_thread(self, thread_id, embeds):
		"""Post a new message inside a thread via webhook."""
		if not self.webhook_url or not thread_id:
			return
		response = _request_with_retry(
			'POST', f'{self.webhook_url}?wait=true&thread_id={thread_id}',
			json={'embeds': embeds}
		)
		if response:
			debug('finding posted to thread', sub='drivers.discord')

	def _send_message(self, embeds):
		"""Post a new top-level Discord webhook message."""
		if not self.webhook_url:
			debug('skipped: no webhook_url configured', sub='drivers.discord')
			return
		response = _request_with_retry('POST', self.webhook_url, json={'embeds': embeds})
		if response:
			debug('message sent', sub='drivers.discord')

	def _get_thread_name(self, runner):
	...
	def _build_runner_embed(self, runner):
	...
	def _build_finding_embed(self, item):
	...
	def _build_finding_channel_embed(self, item, thread_id):
	...
	def _passes_severity_filter(self, item):
		...
		if not severity:
			return True
		try:
			min_idx = SEVERITY_ORDER.index(self.min_severity.lower())
			item_idx = SEVERITY_ORDER.index(severity.lower())
			return item_idx >= min_idx
		except ValueError:
			return True

	def _passes_type_filter(self, item):
		"""Check if a finding passes the finding type filter."""
		if not self.finding_types:
			return True
		return item._type in self.finding_types

	def update_runner(self, runner):
		"""Send or update runner message on Discord."""
		if not self.send_runner_updates:
			return
		embed = self._build_runner_embed(runner)
		thread_id = runner.context.get('_discord_thread_id')

		if runner.has_parent and thread_id:
			own_msg_key = f'_discord_task_msg_{runner.unique_name}'
			own_msg_id = runner.context.get(own_msg_key)
			if own_msg_id:
				self._edit_message(own_msg_id, [embed], thread_id=thread_id)
			else:
				response = _request_with_retry(
					'POST', f'{self.webhook_url}?wait=true&thread_id={thread_id}',
					json={'embeds': [embed]}
				...
				if response:
					...
					runner.context[own_msg_key] = own_msg_id
					debug(f'task message created in thread: {own_msg_id}', sub='drivers.discord')
		else:
			msg_key = '_discord_msg_id'
			msg_id = runner.context.get(msg_key)
			if msg_id:
				self._edit_message(msg_id, [embed])
			else:
				msg_id, channel_id = self._create_message([embed])
				if msg_id:
					runner.context[msg_key] = msg_id
					if self.bot_token and channel_id:
						thread_name = self._get_thread_name(runner)
						thread_id = self._create_thread(channel_id, msg_id, thread_name)
						if thread_id:
							runner.context['_discord_thread_id'] = thread_id

	def update_finding(self, runner, item):
		"""Send finding to Discord."""
		if not self.send_findings:
			return item
		if type(item) not in FINDING_TYPES:
			return item
		if not self._passes_type_filter(item):
			return item
		if not self._passes_severity_filter(item):
			return item
		embed = self._build_finding_embed(item)
		thread_id = runner.context.get('_discord_thread_id')
		if thread_id:
			self._post_to_thread(thread_id, [embed])
			channel_embed = self._build_finding_channel_embed(item, thread_id)
			self._send_message([channel_embed])
		else:
