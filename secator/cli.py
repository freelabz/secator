...
def report_list(ctx, workspace, runner_type, time_delta, show_all, interesting, status):
	...
	# Print paths if piped (only load reports when a filter requires it)
	if ctx.obj['piped_output']:
		if interesting or status_pattern:
			paths = [p for p in paths if _report_matches_filters(p, interesting, status_pattern)]
		if not paths:
	...
	# Load each report once and render the rows that pass the active filters
	shown = 0
	for path in paths:
		try:
